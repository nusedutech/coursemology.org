class Assessment::TrainingSubmissionsController < Assessment::SubmissionsController
  ...
  def edit
    ...
    if @training.realtime_session_groups.count > 0
      ...
      session = @training.sessions.include_std(curr_user_course).started.last || @training.sessions.select(&:recitation_group?).last
      if session.nil?
         redirect_to course_assessment_realtime_session_groups_path,
  ...
  def submit
    ...
    if params[:sq_id] and params[:sid]
      ...
      if !session_question.unlock && !(session.recitation_group? && (session.end_time < Time.now || session.status == false)) # Not recitation ended or closed.
        response = {
            result: false,
  ...
  def recitation_training?
    @training.realtime_session_groups.all?(&:recitation?)
  end
end
