  ...
  def show
    if can?(:participate, Course) || can?(:share, Course)
      ...
      @pending_actions = curr_user_course.pending_actions.includes(:item).to_show.
          select { |pa| pa.item.published? && !pa.item.used_as_realtime? && pa.item.open_at < Time.now }.
  ...
  def import_ivle_student
    ...
    if params["import_ivle_student"] && params["import_ivle_student"]["chose"]
      params["import_ivle_student"]["chose"].each do |e|
          ...
          name = params["import_ivle_student"]["name"][e]=="" ? e : params["import_ivle_student"]["name"][e]
          ...
          unless user_course
            ...
            count_join_course = count_join_course+1
          end
      end
      respond_to do |format|
        format.html { redirect_to course_manage_students_path(@course), notice: ( "Imported #{count_import_student} student(s) to EduTech.") + ("<br />Added #{count_join_course} student(s) to course.") }
      end
    else
