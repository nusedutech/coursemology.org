class Assessment < ActiveRecord::Base
  ...
  scope :mission, -> { where(as_assessment_type: "Assessment::Mission") } do
    def mission_without_realtime
      where("assessments.as_assessment_id not in (?)",
             Assessment::RealtimeSessionGroup.where("mission_id is not null").select(:mission_id).uniq.map(&:mission_id).push(0))
    end
  end
  ...
  amoeba do
    #clone [:questions]
    include_field [:question_assessments,:as_asm_reqs]
    # as_requirements
  end
